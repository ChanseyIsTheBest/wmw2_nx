/* jni_fake.c -- fake JNI environment for the Where's My Water? 2 engine
 *               (libwalaber.so, com.disney.wheresmywater2_goo)
 *
 * The engine does the heavy lifting natively: it renders through GLES 1.1
 * fixed-function, parses its data with a statically-linked jsoncpp, reads its
 * assets straight off disk with fopen(), and plays audio through FMOD Ex. The
 * Java side it calls back into is com.disney.GameLib.Bridge.* -- twenty-one
 * subsystem bridges rather than WMW1's single BaseActivity.
 *
 * This file provides the generic JNIEnv/JavaVM machinery; the WMW2-specific
 * answers live in wmw2_jni.c, which is consulted before the logged default.
...
/* Read a jstring argument without taking a reference. wmw2_jni.c needs this to
 * see the sku / movie path an upcall carried, so it can post the matching
 * answer back with the right identity. Never returns NULL. */
const char *jni_peek_string(void *jstr) { return obj_str(jstr); }
...
/* Trace budget for bring-up.
 *
 * jni_fake only ever logged upcalls it did NOT handle, on the reasoning that a
 * handled call is uninteresting. That reasoning fails the moment the engine
 * aborts inside a call the port answered "correctly": the log then shows
 * nothing at all for the thing that killed it. Trace every upcall for a while
 * after startup, then go quiet so play is not drowned in logging. */
int jni_trace_budget = 400;

static void trace_call(const char *kind, const FakeID *id) {
  /* Video and movie traffic is always logged, budget or not.
   *
   * The engine plays eight location intros out of assets/Water/Movies/ and
   * there is no decoder here, so the port answers "already finished". When a
   * cutscene comes up black the first thing anyone needs to know is which of
   * these the engine actually called -- and the general trace budget is long
   * exhausted by the time a level is reached, so without this the log says
   * nothing at all about it. */
  if (id && (strstr(id->name, "Movie") || strstr(id->name, "Video"))) {
    debugPrintf("  jni-> %s %s%s  [video]\n", kind, id->name, id->sig);
    debugLogFlush();
    return;
  }
  if (jni_trace_budget <= 0) return;
  jni_trace_budget--;
  debugPrintf("  jni-> %s %s%s\n", kind, id->name, id->sig);
  if (jni_trace_budget == 0)
    debugPrintf("  jni-> (trace budget exhausted; further upcalls not logged)\n");
}
...
static juint call_numeric(const FakeID *id) {
  ...
  if (forced_lookup(id->name, &v))
    return (juint)v;
  if (wmw2_jni_numeric(id->name, id->sig, &v)) // wmw2_jni.c answers
    return (juint)v;
  log_call("num", id);
  return 0;
}
...
static float call_float(const FakeID *id) {
  trace_call("flt", id);
  float v = 0.0f;
  if (wmw2_jni_float(id->name, id->sig, &v))
    return v;
  log_call("flt", id);
  return 0.0f;
}
...
static void *call_object(const FakeID *id) {
  trace_call("obj", id);
  const char *s = wmw2_jni_string(id->name, id->sig); // wmw2_jni.c answers
  if (s)
...
/* WMW2's void upcalls carry arguments that matter -- jniPlayMovie needs the
 * path, jniMigsRequestStoreGetSingleStoreItem needs the sku, and the answer the
 * port posts back depends on them. WMW1's hook took only (name, sig) because
 * none of its void calls had a payload worth reading; that is no longer true,
 * so the whole argument list is handed through. */
static void call_void_v(const FakeID *id, va_list va) {
  trace_call("void", id);
  if (wmw2_jni_void(id->name, id->sig, va))
    return;
  log_call("void", id);
}

/* There is deliberately no zero-argument variant of the above. Building an
 * "empty" va_list by memset and reading from it is undefined behaviour, and the
 * hook has no way to know an argument is absent -- it would read whatever
 * happens to sit in the register save area. Every path into the void dispatch
 * has a real argument list, so all of them go through call_void_v. */
...
// --- object arrays ----------------------------------------------------------
// New for WMW2. Two entry points need them, in opposite directions:
//   jniGetRewardVideoIdsToPreload()[Ljava/lang/String;  -- the ENGINE builds an
//       array and returns it, so it calls NewObjectArray + SetObjectArrayElement.
//       (This is also the only reason libwalaber.so contains a FindClass string
//       at all: "java/lang/String".)
//   jniNotifyNewMigsInfoStoreProductToRestore([Ljava/lang/String;)V -- the PORT
//       builds the array and the engine reads it back with GetArrayLength +
//       GetObjectArrayElement.

static void *make_obj_array(int len) {
...
static void *j_NewObjectArray(void *env, int len, void *cls, void *init) {
  (void)env; (void)cls;
  FakeObjArray *a = make_obj_array(len);
  if (init)
...
void *jni_make_string_array(const char *const *items, int count) {
  FakeObjArray *a = make_obj_array(count);
  for (int i = 0; i < count; i++)
    a->items[i] = jni_make_string(items[i] ? items[i] : "");
  return a;
}
