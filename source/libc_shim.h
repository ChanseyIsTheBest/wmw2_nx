...
struct bionic_stat;
/* Synthetic directory descriptors -- see open_fake() in libc_shim.c. */
int wmw_is_dir_fd(int fd);

/* Alias descriptors: a second open of a file already held. wmw_real_fd()
 * translates one to the underlying descriptor and passes anything else
 * through, so shims can call it unconditionally. */
int wmw_is_alias_fd(int fd);
int wmw_real_fd(int fd);
int wmw_release_alias_fd(int fd);
void wmw_invalidate_aliases_for(int real_fd);
