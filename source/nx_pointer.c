...
/* Overall cursor size, applied to both the built-in arrow and a user cursor.png.
 *
 * Both are otherwise sized from the render height, so they stay proportionate
 * on any target; this is a plain taste multiplier on top of that. 0.75 was
 * chosen because the default covered too much of a portrait 720x1280 playfield
 * -- the game is played by dragging through dirt and the cursor was hiding the
 * thing being dug. Change this one number to resize both. */
#define NXP_CURSOR_SCALE 0.75f
...
void nxp_draw(void) {
  ...
  if (s_cursor_tex) {
    ...
    const GLfloat sc = NXP_CURSOR_SCALE * ((GLfloat)s_cfg.screen_h / 1080.0f);
    ...
    const GLfloat sc = NXP_CURSOR_SCALE * 2.4f * ((GLfloat)s_cfg.screen_h / 1080.0f);
    glPushMatrix();
